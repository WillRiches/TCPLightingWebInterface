<?php
	...
	if (strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
		...
		if ($sceneID == '' || $action == '') {
			echo json_encode([
                'error' => 'Invalid SceneID or Action'
            ]);
		...
		if ($action == 'save') {
			...
			$cmd .= '<name>'.$name.'</name>';
			$cmd .= '<type>'.$type.'</type>';
			$cmd .= '<islocal>1</islocal>'; //Todo: Why 'unknown...'?
		...
		if ($action == 'delete') {
			...
			echo json_encode( array('success' => 1, 'scene' => $sceneID, 'fx' => $action, 'resp' => xmlToArray(getCurlReturn($CMD))));
		}
